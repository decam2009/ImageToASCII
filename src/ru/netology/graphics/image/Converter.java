...
public class Converter implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        if (currentWidth/currentHeight > maxRatio){
            throw new BadImageSizeException(maxRatio, currentWidth/currentHeight);
        }
        ...
        int newWidth = (maxWidth > currentWidth ? currentWidth : maxWidth);
        int newHeight = (currentHeight > maxHeight ? currentHeight / (currentWidth / maxWidth): currentHeight);
        ...
        for (int i = 0; i < newHeight; i++) {
            for (int j = 0; j < newWidth; j++) {
                convertedImage.append("" + result[i][j] + result[i][j]);
            }
